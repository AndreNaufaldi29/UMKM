...
echo "¸êÄ [1/4] Building production image secara lokal..."
docker build --target runner -t ${REGISTRY_IMAGE} .

echo "üì§[ 2/4] Pushing image ke Docker Registry (${REGISTRY_IMAGE})..."
docker push ${REGISTRY_IMAGE}

echo "üì°˚ 3_4] Mengirim file konfigurasi ke server production..."
ssh ${SERVER_USER}@${SERVER_HOST} "mkdir -p ${SERVER_DIR}"
scp docker-compose.prod.yml .env ${SERVER_USER}@${SERVER_HOST}:${SERVER_DIR}/

echo "üîõ [4/4] Pulling image terbaru & me-restart container di server production..."
ssh ${SERVER_USER}@${SERVER_HOST} "cd ${SERVER_DIR} && \
    cp docker-compose.prod.yml docker-compose.yml && \
    DOCKER_IMAGE=${REGISTRY_IMAGE} docker compose pull && \
    DOCKER_IMAGE=${REGISTRY_IMAGE} docker compose up -d --remove-orphans"

echo "‚úÖ Deployment ke server production BERHASIL!"
